...
const CASI = [
  ...
  },
  {
    ...
    opzioni: ["VINCE IL CREATORE (Ha il copyright)", "VINCONO GLI UTENTI (Dominio pubblico)"],
    corretta: 0,
...
export default function Tribunale() {
  ...
  if (mostraRisultato) {
    return (
      <main className="min-h-screen bg-neutral-950 flex items-center justify-center p-4 md:p-6">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-neutral-900 border border-neutral-800 p-6 md:p-8 rounded-2xl max-w-lg w-full text-center shadow-2xl">
          <Scale className="w-20 h-20 md:w-24 md:h-24 mx-auto text-yellow-500 mb-4 md:mb-6" />
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">L'Udienza è Tolta</h2>
          <p className="text-lg md:text-xl text-neutral-300 mb-6 md:mb-8">
            Hai emesso sentenze corrette su <span className="text-yellow-500 font-bold">{punteggio}</span> casi su {CASI.length}.
          </p>
          <div className="w-full bg-neutral-800 rounded-full h-3 md:h-4 mb-6 md:mb-8">
            <motion.div 
              initial={{ width: 0 }} 
              animate={{ width: `${(punteggio / CASI.length) * 100}%` }} 
              transition={{ duration: 1, delay: 0.5 }}
              className="bg-yellow-500 h-3 md:h-4 rounded-full"
            />
          </div>
          <Link href="/" className="inline-block bg-yellow-600 hover:bg-yellow-500 text-white font-semibold py-3 px-6 md:py-4 md:px-8 rounded-xl transition-colors text-sm md:text-base">
            Chiudi il Fascicolo
          </Link>
        </motion.div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-white py-8 md:py-12 px-4 md:px-6 overflow-hidden">
      <div className="max-w-3xl mx-auto">
        <Link href="/" className="inline-flex items-center text-yellow-500 hover:text-yellow-400 mb-6 md:mb-8 transition-colors text-sm md:text-base">
        ...
        >
          <div className="bg-neutral-800 px-4 py-3 md:px-6 md:py-4 border-b border-neutral-700 flex justify-between items-center text-xs md:text-sm">
          ...
          <div className="p-5 md:p-8">
            <h2 className="text-2xl md:text-3xl font-bold mb-3 md:mb-4">{caso.titolo}</h2>
            ...
            <AnimatePresence mode="wait">
              {sceltaUtente === null ? (
                <motion.div key="opzioni" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                  {caso.opzioni.map((opzione, idx) => (
                    <motion.button
                      ...
                      className="p-4 md:p-6 rounded-xl border-2 border-neutral-700 hover:border-yellow-500 hover:bg-neutral-800 transition-colors text-left font-bold text-base md:text-lg"
                    >
                      {opzione}
                    </motion.button>
                  ))}
                ...
                <motion.div key="risultato" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 md:space-y-6">
                  <div className={`p-4 md:p-6 rounded-xl border-2 flex flex-col md:flex-row items-start gap-3 md:gap-4 ${sceltaUtente === caso.corretta ? 'bg-green-950/40 border-green-500/50 text-green-100' : 'bg-red-950/40 border-red-500/50 text-red-100'}`}>
                    {sceltaUtente === caso.corretta ? <CheckCircle2 className="w-8 h-8 md:w-10 md:h-10 shrink-0 text-green-500" /> : <AlertOctagon className="w-8 h-8 md:w-10 md:h-10 shrink-0 text-red-500" />}
                    <div>
                      <h3 className="font-bold text-xl md:text-2xl mb-2 md:mb-3">
                        {sceltaUtente === caso.corretta ? "Sentenza Corretta!" : "Errore di Giudizio!"}
                      </h3>
                      <p className="text-base md:text-lg leading-relaxed opacity-90">{caso.spiegazione}</p>
                    </div>
                  </div>
                  
                  <button
                    onClick={avanti}
                    className="w-full bg-white text-black hover:bg-neutral-200 font-bold py-3 md:py-4 rounded-xl transition-colors text-base md:text-lg"
                  >
